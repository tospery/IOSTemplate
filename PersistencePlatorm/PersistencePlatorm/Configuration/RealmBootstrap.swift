...
/// 集中管理 Realm 的 schema 版本、`objectTypes` 与迁移逻辑，并同步到 `Realm.Configuration.defaultConfiguration`。
/// 须在任意 `Realm(...)` / `@Shared` Realm 访问之前调用 ``installDefaultRealmConfiguration()``（例如在 `App.init` 中）。
public enum RealmBootstrap {

    /// 当前应用 schema 版本；迁移模板 `default-v0.realm` 等可使用更低版本号，由 ``RealmBootstrap/writeMigrationTemplateFile(fileURL:schemaVersion:seed:)`` 写入。
    public static var currentSchemaVersion: UInt64 = 0

    /// 参与持久化的 `Object` 类型（显式列举，避免遗漏子类型导致迁移不完整）。
    public static let objectTypes: [ObjectBase.Type] = [
        RMLanguage.self,
        RMPreference.self,
        RMUser.self,
        RMLogin.self,
    ...
    public static func makeAppConfiguration(fileURL: URL? = nil) -> Realm.Configuration {
        Realm.Configuration(
            fileURL: fileURL ?? Realm.Configuration.defaultConfiguration.fileURL,
            schemaVersion: currentSchemaVersion,
            migrationBlock: { migration, oldSchemaVersion in
                migrate(migration: migration, oldSchemaVersion: oldSchemaVersion)
            },
            objectTypes: objectTypes,
            seedFilePath: self.seedFilePath(for: currentSchemaVersion)
        )
    }

    /// 将应用配置写回全局默认，供 `Repository`、`@Shared(.preference)` 等使用 `.defaultConfiguration` 的路径一致。
    public static func installDefaultRealmConfiguration() {
        Realm.Configuration.defaultConfiguration = makeAppConfiguration()
    }
    
    public static func prepareSeedFileIfNeed() {
        let destinationUrl = self.seedFilePath(for: currentSchemaVersion)
        let fileName = destinationUrl.lastPathComponent.components(separatedBy: ".").first!
        ...
        if bundleUrl == nil {
            let string = """
                ...
                """
            let languages = [Domain.Language].init(JSONString: string)!.map { $0.asRealm() }
            if FileManager.default.fileExists(atPath: destinationUrl.path) {
                try! FileManager.default.removeItem(at: destinationUrl)
            }
            let configuration = Realm.Configuration(fileURL: destinationUrl)
...
//    public static func prepareSeedFileIfNeed() {
//        for oldSchemaVersion in 0..<(currentSchemaVersion + 1) {
//            let destinationUrl = self.seedFilePath(for: oldSchemaVersion)
//            let fileName = destinationUrl.lastPathComponent.components(separatedBy: ".").first!
//            if !FileManager.default.fileExists(atPath: destinationUrl.path) {
//                let bundleUrl = Bundle.main.url(forResource: fileName, withExtension: "realm")!
//                try! FileManager.default.copyItem(at: bundleUrl, to: destinationUrl)
...
//    }
    ...
    private static func seedFilePath(for schemaVersion: UInt64) -> URL {
        let defaultURL = Realm.Configuration.defaultConfiguration.fileURL!
        let defaultParentURL = defaultURL.deletingLastPathComponent()
        let fileName = "seed-v\(schemaVersion)"
