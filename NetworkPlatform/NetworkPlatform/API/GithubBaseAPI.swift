...
enum GithubBaseAPI {
    ...
    // MARK: search
    case searchRepos(keyword: String, language: Language?, sort: String, order: OrderType, pageIndex: Int, pageSize: Int)
    case searchUsers(keyword: String, sort: String, order: OrderType, pageIndex: Int, pageSize: Int)
    // MARK: other
...
extension GithubBaseAPI: TargetType {
    ...
    var path: String {
        ...
        case .feedback: return "/repos/\(Author.owner)/\(Author.repo)/issues"
    ...
    var task: Task {
        ...
        case .searchUsers(let keyword, let sort, let order, let pageIndex, let pageSize):
            parameters[Parameter.searchKey] = keyword
            parameters[Parameter.sort] = sort
            parameters[Parameter.order] = order.rawValue
            parameters[Parameter.pageIndex] = pageIndex
            parameters[Parameter.pageSize] = pageSize
        case .searchRepos(let keyword, let language, let sort, let order, let pageIndex, let pageSize):
            if (language?.id.isEmpty ?? true) || (language?.id == Language.any.id) {
                parameters[Parameter.searchKey] = keyword
            } else {
                parameters[Parameter.searchKey] = "\(keyword) language:\(language?.id ?? "")"
            }
            parameters[Parameter.sort] = sort
            parameters[Parameter.order] = order.rawValue
            parameters[Parameter.pageIndex] = pageIndex
