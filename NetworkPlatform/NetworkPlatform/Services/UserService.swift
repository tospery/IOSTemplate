...
final class UserService: Domain.UserService {
    ...
    func user(token: String) -> AnyPublisher<Domain.User, any Error> {
        multiNetworking.requestObject(
            MultiTarget.init(AuthingAPI.user(token: token)),
            type: Domain.User.self
        )
    }
...
//        multiNetworking.requestArray(
//            MultiTarget.init(
//                TrendingAPI.users(language: language, since: since)
//            ),
//            type: User.self
//        )
//        .map { $0.map { $0.copyWith(pageType: .trendingUsers) } }
//        .eraseToAnyPublisher()
//    }
//    
...
//            MultiTarget.init(
//                GithubBaseAPI.searchUsers(keyword: keyword, sort: sort, order: order, pageIndex: pageIndex, pageSize: pageSize)
//            ),
//            type: User.self
